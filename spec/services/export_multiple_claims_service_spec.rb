  ...
  describe '#call' do
    ...
    context 'with secondary claimants from csv file' do
      ...
      it 'queues the header worker when done with the data from the header presenter' do
        ...
        expect(mock_header_worker).to have_received(:perform).with(match(/\d{7}\/\d{4}/), example_export.resource.primary_respondent.name, match_array((1000001..(1000001 + example_export.resource.secondary_claimants.length)).to_a.map(&:to_s)), 'Manchester_Multiples', example_export.id, true)
      end
      ...
      it 'queues the worker 11 times with the data from the presenter' do
        ...
        aggregate_failures 'validating calls' do
          expect(mock_worker_calls.first).to eql(['{"claim"=>"1"}', 'Manchester', example_export.id, 11, true, true])
          expect(mock_worker_calls[1..-1]).to eql presented_values[1..-1].map {|data| [data, 'Manchester', example_export.id, 11, false, true]}
        end
      end
